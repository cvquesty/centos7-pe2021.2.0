...
######################
  ...
  master.vm.provider :virtualbox do |settings|
    ...
    settings.name = "c7master_2021.2.0"
    settings.cpus = 2
  end
  ...
  # Set the PE Role for this node and run post-install configuration
...
###################
  ...
  development.vm.provider :virtualbox do |settings|
    settings.memory = 512
    settings.name = "c7development_2021.2.0"
    settings.cpus = 1
  end
...
###################
  ...
  production.vm.provider :virtualbox do |settings|
    settings.memory = 512
    settings.name = "c7production_2021.2.0"
    settings.cpus = 1
  end
